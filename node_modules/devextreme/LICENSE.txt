...
LICENSEE MAY CREATE their own js bundles based on js modules using tools in the SOFTWARE DEVELOPMENT PRODUCT(S) distribution or other tools, and REDISTRIBUTE them under this AGREEMENT. A module MAY BE USED to create a custom bundle only if its license header allows a LICENSEE to use the module with LICENSEE’s subscription.
